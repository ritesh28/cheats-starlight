...
| Network   | `docker network create --driver bridge isolated_network` | create a network                                                                                    |
...
- `depends_on` property within a service: we are able to specify that one container should wait for another container to start up first
  ...
  product-service: ##--> name of the service
    ...
    volumes: ##--> list of volumes
      ...
      - type: bind ##--> Second item. Alternative short-hand syntax: './product-service:/app'
        ...
        target: /app
    depends_on: product-db ##--> instruct the Docker to wait for 'product-db' to start up first
  inventory-service:
    build:
