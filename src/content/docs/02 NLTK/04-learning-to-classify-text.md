...
- Decision Tree:
  - Its a flowchart that selects labels for input values. This flowchart consists of:
    - decision nodes: which check feature values. Each node contains a condition that checks one of the input value's features, and selects a branch based on that feature's value
    - leaf nodes: which assign labels
- Naive Bayes Classifier:
  - Its build on the assumption that every input value is generated by first choosing a class label, and then generating each feature, entirely independent of every other feature
    - Of course, this assumption is unrealistic; features are often highly dependent on one another
    - Thats why is it also called **generative process**
- Maximum Entropy Model (MaxEnt):
  - MaxEnt and the Naive Bayes Classifier are both probabilistic text models
  - Naive Bayes is a **generative model** that calculates the joint probability of the text and the label
  - MaxEnt is a **conditional model** that calculates the conditional probability of the label given the text
  - Unlike Naive Bayes, MaxEnt does not assume that text features (like neighboring words or prefixes) are independent of each other
  - Its a iterative process to find the set of **feature weights** that maximizes the probability of the training set
