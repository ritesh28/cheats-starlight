...
## Compress & Isolate - Managing context at scale

![management](./context-engineering-management.drawio.svg)

- WSCI Framework:
  - WRITE: Memory short term persistent (scratchpad -> `TASK.md`)
  - SELECT: RAG
  - COMPRESS: Squeeze the context window
  - ISOLATE: sub-agents; `AGENTS.md`
- Types of compression: (SEE INFOGRAPHIC)
  - Summarization (Agents prefer this)
  - Tool Result Compression (Agents prefer this as well)
  - Priority-based trimming: Priority is assigned as per pre-defined rules (No LLM needed here)
  - Hierarchical Compression: Semantic meaning of each part of context is generated by LLM. The part with higher semantic value is less compressed and vis-a-versa
- IRR (Instruction Retention Ratio): Its the ratio between how many key fact/data were present in the initial message and how many are remaining in the summarization
  - You can use LLM to generate (say 20) key facts from the initial message and then, verify how many are present in the summarization (again using LLM)
- Compaction Thresholds - Compact (say at 80% of context window) & Emergency Trim (a higher number; say 95%):
  - Auto-fire compression when capacity threshold is reached/crossed
- Sub-agents: For e.x. Claude code (orchestrator agent) spun 2 sub-agent - A1 & A2:
  - Context window is not shared between A1 & A2 - A1 or A2 does not know about each other
  - Orchestrator may provide same token (like system prompt) to both sub-agent - duplicate copies and not shared (at the end of day, its a LLM API call)
- | Feature  | ClAUDE.md                                                       | agents.md                                                                                |
  | -------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
  | Read by  | Main agent (your chat session)                                  | Sub-agents (spawned by Agent tool)                                                       |
  | Scope    | Whole project                                                   | Specific directory + children                                                            |
  | Contains | Project overview,Build commands,Coding conventions,Global rules | Domain-specific architecture,Key files and patterns,Common pitfalls,Testing instructions |
  | When     | Always Loaded                                                   | Only when sub-agent is spawned for work in that directory                                |
  | Analogy  | Company empLoyee handbook                                       | Department onboarding doc (e.x. - marketing,engineering,sales)                           |
- Contract-First Pattern (SEE INFOGRAPHIC):
